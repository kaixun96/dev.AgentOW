...
- Treat manual formatting of localized numbered placeholders as a mandatory prior-art trigger. Sweep changed TypeScript/TSX for patterns such as `.replace('{0}', value)`, placeholder-matching regular expressions, and local `formatString` helpers; account for every match. Prefer the repository's `Text.format` or `StringHelper.format` unless source inspection proves their substitution semantics are incompatible.
