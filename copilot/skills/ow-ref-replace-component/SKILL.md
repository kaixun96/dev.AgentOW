...
- For publishable packages, create the required Rush change file before push:
  ...
  Never manually create the JSON under `common/changes/`. The file must be generated by the
  approved `rush change` command before its generated message or metadata may be edited. If the
  command fails or hangs, repair it or report the blocker instead of hand-writing a fallback.
