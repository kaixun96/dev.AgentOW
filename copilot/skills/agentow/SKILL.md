...
- Detheme classification for any theme-affecting SharePoint UI: app-chrome-invoked,
  ...
  Read `${CLAUDE_PLUGIN_ROOT}/skills/detheme/SKILL.md` and record the required treatment.
...
2. **Write the code** yourself, following the planner's "patterns to follow". Surgical changes only — every line traces to the request.
   ...
   - For any theme-affecting SharePoint UI, read
     `${CLAUDE_PLUGIN_ROOT}/skills/detheme/SKILL.md`, confirm the planned surface
...
- If screenshots are missing, visual validation failed, or the evaluator says the FIC Playwright harness was unavailable, treat the evaluator result as **FAIL** even if code inspection passed.
...
2. **Visual validation for UI changes:** use the evaluator's FIC Playwright/Heft spec with the local `rush start` debug link first because it is available immediately after implementation. If local debug validation captures BEFORE/AFTER successfully, attach those screenshots to the PR description. Only fall back to the same FIC spec with `ow-pr-debug-query` / `finalValidationMode=pr-cdn-fic` when local debug validation fails for a route-specific reason (for example localhost cert/assembly-load failure, missing local dev server, or a surface that only reproduces against PR CDN). Playwright MCP and browser profiles are not AgentOW validation routes.
