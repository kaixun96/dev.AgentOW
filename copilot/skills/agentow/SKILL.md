...
6. **Dev server / debug link:** follow the feature context docs for the surface's verification contract. For UI-visible changes, prefer `ow-start` + `ow-debuglink` before PR validation builds finish; if the context docs require additional guards, execute those guards and record the result.
   - If the debug page loads but the app shell does not mount, read the trace before retrying. A key recurring case in odsp-web is **branch drift**: the page itself returns 200 and the local debug loader/manifests are reachable, but RequireJS then fails to load shared bundles such as `sp-page-chrome`, `office-ui-fabric-react-bundle-internal`, or `uifabric-variants-bundle`. Treat that pattern as "your branch is probably too old for the current shell", not as a selector or screenshot problem. Sync the feature branch with the latest `origin/main` (rebase or merge per repo policy), rebuild, and only then retry the same screenshot route.
