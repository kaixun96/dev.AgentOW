...
2. **Final PR visual validation for UI changes:** after PR creation, use `ow-pr-debug-query` to fetch the PR's `SP-Client Validation` CDN debug query from PR threads. Dispatch the evaluator again with `finalValidationMode=pr-cdn-fic`, `prId`, `prUrl`, `debugQuery`, `sessionDir`, and the visual surface trace. The evaluator must capture final BEFORE/AFTER screenshots using the PR CDN query (or explicitly fail with the reason). Localhost/rush-start screenshots are not final PR screenshots when a PR CDN query exists.
