...
1. **Push** the branch and **create the draft PR:** `ow-pr-create` with title (from the plan spec) and description (Summary + Changes — no generic auto-generated "Testing" section). When changed files include `sp-client/`, add a concise `Buddy Test Cases` section from actual evaluator evidence with Given/When/Then scenarios for the happy path, rollback/disabled state when gated, and a relevant edge case. Never invent a manual result.
