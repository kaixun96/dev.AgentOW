...
For A11y bug fixes, apply the **A11y scope boundary** in
`skills/ow-review/references/sharepoint-design-system-and-ux-components.md` before generic
component-fit or import-package findings. Do not require an unchanged Fluent/SPDS control to
migrate merely because this diff fixes focus, contrast, ARIA, headings, names, or keyboard
behavior. Do not invoke the ReplaceComponent skill or make migration a Critical/Important
finding without an explicit user request or concrete evidence that migration is necessary for
the accessibility acceptance criteria. Record migration-only inventoried checks as not applicable
with unchanged-import evidence; preserve all directly relevant correctness and regression checks.
