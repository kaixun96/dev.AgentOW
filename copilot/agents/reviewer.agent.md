...
| `accessibility.md` | Added or changed interactive or semantic UI: controls, links, forms, dialogs, flyouts, menus, focus/keyboard behavior, visibility toggles, accessible names/roles/states, DOM structure that affects reading or tab order, or a rendered collection that loads, appends, refreshes, searches, filters, sorts, groups, pages, retries, or replaces results. Load this reference for those async collection workflows even when the diff contains no announcement API; missing announcement behavior is itself under review. | Data providers, API clients, models, pure formatting/business logic, non-rendered hooks, visual-token-only changes with unchanged semantics, or tests that do not alter product interaction |
