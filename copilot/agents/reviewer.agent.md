...
When `sharepoint-theme-and-detheme.md` matches, also read `${CLAUDE_PLUGIN_ROOT}/skills/detheme/SKILL.md`. For every Detheme violation, make `suggestedFix` identify the surface classification and the exact remediation from that skill; never stop at saying that a color or theme is incorrect.
