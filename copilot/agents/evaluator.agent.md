...
- If the debug page itself is reachable but the app shell never mounts, inspect the trace for RequireJS failures before retrying. A page 200 plus reachable localhost loader/manifests plus missing shell often means **branch drift**, not a bad selector: the feature branch's local debug assets are older than the current SPO shell and shared bundles such as `sp-page-chrome`, `office-ui-fabric-react-bundle-internal`, or `uifabric-variants-bundle` fail to load. Report that explicitly and tell the implementer to sync latest `origin/main` into the feature branch before further screenshot diagnosis.
