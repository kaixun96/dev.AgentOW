...
const localizationFormattingSnippets = [
  ...
  "When the PR adds unnecessary placeholder-lock metadata, report a non-blocking Nit",
  'Nit: The placeholder-lock metadata `{Locked="{0}"}` is unnecessary.',
  "adapt the metadata and placeholder names to the actual occurrence",
  "Please remove it while keeping the translator comment",
  "Do not suggest replacing a placeholder lock with `{Locked}` unless",
  "cover all newly added occurrences in the current diff; do not flag unchanged pre-existing locks",
  "Preserve the placeholder explanation and any legitimate whole-string approval lock",
...
const accessibilitySnippets = [
  ...
  "use Fluent's memoized provider-backed",
  "provide `AriaLiveAnnouncer` at the owning application or",
...
const checks = [
  ...
  },
  {
    ...
    snippets: [
      ...
      "GDI/MSS",
      "WASAPI/PyAudioWPatch",
      "1.2 seconds of continuous silence",
      "image variance",
      "audio RMS and peak",
      "Browser-only capture is insufficient",
      "An MP4 file's existence alone is not evidence",
  ...
  },
  {
    file: "copilot/skills/ow-ref-monorepo/SKILL.md",
    snippets: [
      "Never manually create or hand-write a Rush change file",
      "Run the",
      "`rush change` command first",
      "do not construct the JSON as a fallback"
    ]
  },
  {
    file: "copilot/skills/agentow/SKILL.md",
    snippets: [
      "Every change file must originate",
      "never",
      "hand-write `common/changes/**/*.json`",
  ...
  },
  {
    file: "copilot/skills/ow-ref-replace-component/SKILL.md",
    snippets: [
      "Never manually create the JSON under `common/changes/`",
      "generated by the",
      "instead of hand-writing a fallback"
    ]
  },
