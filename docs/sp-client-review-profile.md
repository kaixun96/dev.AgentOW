...
- Apply the shared rollout exceptions from `common-review-issues.md`: do not force reordering of commutative pure predicates, keep hooks unconditional and place gate checks inside hook callbacks when needed, and treat one session's Flight/KS value as stable unless rollout config is explicitly reloaded.
- Identify the rollback strategy and whether KS, flight/Feature, experiment, or an existing gate is appropriate.
- Check GUID case: use lowercase for `sp-client`; most other projects use uppercase. For an added KS, read `.ai/killswitches/skills/killswitches/SKILL.md` in the odsp-web repository and apply every applicable killswitch rule in it, not only its format guidance. If that path is absent, resolve its replacement only under `.ai/killswitches/**/SKILL.md`; do not silently skip the skill or invent its requirements.
