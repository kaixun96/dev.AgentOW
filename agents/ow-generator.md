...
- **Default to ZERO comments — triple-check necessity before adding ANY.** Before you write a comment, ask: would a competent reader genuinely fail to understand this code WITHOUT it? If the identifier/structure already says it, write NO comment — a redundant comment is noise reviewers skip past. Only a real, non-obvious *why* earns a comment; when in doubt, leave it out. If one is truly needed, keep it minimal (next rule).
